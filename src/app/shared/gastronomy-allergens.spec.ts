...
describe('gastronomy allergens', () => {
  ...
  it('has a profile for every published gastronomy card', () => {
    ...
    expect(gastronomyCards.length).toBe(130);
    expect(missingProfiles).toEqual([]);
  });

  it('normalizes accents and applies guide-specific recipe variants', () => {
    expect(dishAllergenProfile('Tortillitas de camarones')?.contains)
      .toContain('crustaceos');
    expect(
      dishAllergenProfile(
        'Tortillitas de camarones',
        'europa/espana/andalucia/cadiz/trebujena'
      )?.possible
    ).toContain('gluten');
  });
